...
pub fn register_preload(lua: &Lua) -> LuaResult<()> {
    ...
    preload.set(
        ...
        lua.create_function(|lua, ()| {
            ...
            let get_item_size = lua.create_function(move |lua, item: LuaValue| {
                ...
                if let LuaValue::String(info_str) = info {
                    ...
                    let info_w: f64 = match &font {
                        LuaValue::Table(t) => t.call_method("get_width", info_str.to_str()?.to_string())?,
                        LuaValue::UserData(ud) => ud.call_method("get_width", info_str.to_str()?.to_string())?,
                        _ => 20.0,
                    };
                    lw += px + info_w;
                }
            ...
            let update_items_size = lua.create_function(move |lua, (items, update_binding): (LuaTable, bool)| {
                let style: LuaTable = require_table(lua, "core.style")?;
                let padding: LuaTable = style.get("padding")?;
                let px: f64 = padding.get("x")?;
                    ...
                    if update_binding && !is_divider {
                        if let LuaValue::Table(ref item_tbl) = item {
                            ...
                            if let LuaValue::String(_) = cmd {
                                let binding: LuaValue = keymap.call_function("get_binding_display", cmd)?;
            ...
            context_menu.set("show", {
                ...
                lua.create_function(move |lua, (this, x, y, items, rest): (LuaTable, f64, f64, LuaTable, LuaMultiValue)| {
                    ...
                    for i in 1..=len {
                        ...
                        let should_add = if is_divider {
                            ...
                            if matches!(cmd, LuaValue::Nil) {
                                ...
                                let valid: bool = command.call_function("is_valid", (s.to_str()?.to_string(), args_list))?;
                                valid
                            } else {
                                true
                            }
            ...
            context_menu.set(
                "on_mouse_pressed",
                lua.create_function(|lua, (this, button, x, y): (LuaTable, LuaValue, f64, f64)| {
            ...
            context_menu.set(
                "on_mouse_released",
                lua.create_function(|_lua, (this, _button, _x, _y): (LuaTable, LuaValue, f64, f64)| {
                    let visible: bool = this.get("visible")?;
                    if !visible { return Ok(LuaValue::Boolean(false)); }
                    Ok(LuaValue::Boolean(true))
                })?,
            ...
            context_menu.set(
                ...
                lua.create_function(|lua, (this, item): (LuaTable, LuaTable)| {
                    ...
                    if let LuaValue::String(s) = cmd {
                        ...
                        for v in args { call_args.push(v); }
                        command.call_function::<()>("perform", LuaMultiValue::from_vec(call_args))?;
            ...
            context_menu.set("focus_previous", {
                ...
                lua.create_function(move |lua, this: LuaTable| {
                    ...
                    let new_sel = if selected == -1 || selected == 1 { n } else { selected - 1 };
            ...
            context_menu.set("focus_next", {
                ...
                lua.create_function(move |lua, this: LuaTable| {
                    ...
                    let n = items.raw_len() as i64;
                    let selected: i64 = this.get("selected")?;
                    let new_sel = if selected == -1 || selected == n { 1 } else { selected + 1 };
                    this.set("selected", new_sel)?;
                    let divider: LuaTable = lua.registry_value(&div_k)?;
                    let sel_item: LuaValue = items.raw_get(new_sel)?;
                    let is_div = match &sel_item {
            ...
            context_menu.set(
                ...
                lua.create_function(|_lua, this: LuaTable| {
                    ...
                    if visible {
                        ...
                        this.call_method::<()>("move_towards", (this.clone(), "height", items_height))?;
                    }
                    Ok(())
                })?,
            )?;
            ...
            context_menu.set("draw", {
                ...
                lua.create_function(move |lua, this: LuaTable| {
                    ...
                    let items: LuaValue = this.get("items")?;
                    if matches!(items, LuaValue::Nil) { return Ok(()); }
                    let items = match items { LuaValue::Table(t) => t, _ => return Ok(()) };
                    ...
                    loop {
                        ...
                        let item = match vals.next() { Some(v) => v, _ => break };
                        let x: f64 = match vals.next() { Some(LuaValue::Number(n)) => n, _ => break };
                        let y: f64 = match vals.next() { Some(LuaValue::Number(n)) => n, _ => break };
                        let w: f64 = match vals.next() { Some(LuaValue::Number(n)) => n, _ => break };
                        let h: f64 = match vals.next() { Some(LuaValue::Number(n)) => n, _ => break };
                        ...
                        if is_divider {
                            draw_rect.call::<()>((x, y + divider_padding * scale, w, divider_width, style_divider.clone()))?;
                        } else {
                            let i = match i_val {
                            ...
                            if let LuaValue::Table(ref item_tbl) = item {
                                ...
                                common.call_function::<LuaValue>(
                                    "draw_text",
                                    (font.clone(), style_text.clone(), text, "left", x + px, y, w, h),
                                )?;
                                let info: LuaValue = item_tbl.get("info")?;
